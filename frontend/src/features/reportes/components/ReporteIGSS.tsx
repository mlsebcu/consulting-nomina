...
import type { FilaIGSS } from "../Reportes.types";
...
export function ReporteIGSS() {
    const [periodos, setPeriodos] = useState<Periodo[]>([]);
    const [periodoId, setPeriodoId] = useState<number | null>(null);
    const [lista, setLista] = useState<FilaIGSS[]>([]);
    ...
    async function generar() {
        ...
        setCargando(true);
        try {
            const data = await reportesApi.igss(periodoId);
            setLista(data.lista);
            setTotal(data.total.totalIGSS);
            setBuscado(true);
        } catch {
    ...
    return (
        <Card>
            <CardHeader>
                ...
                <CardDescription>
                    Detalle de descuentos laborales del IGSS por período.
                </CardDescription>
            </CardHeader>
                            ...
                            <TableBody>
                                {lista.length === 0 ? (
                                    <TableRow>
                                        ...
                                        >
                                            No hay descuentos de IGSS en este
                                            período
                                        </TableCell>
                                    </TableRow>
                                ) : (
